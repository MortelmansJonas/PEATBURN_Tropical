...
for i in range(len(fire_data.latitude_I)):
    lat_diffs = abs(lats - fire_data['latitude_P'][i])
    lon_diffs = abs(lons - fire_data['longitude_P'][i])
...
annual_ignitions = annual_ignitions / 81 # To get per km2, as each pixel is 81 km2
...
# Fig 1: The 2015 peatland distribution
...
ax[0].set_title("Peatland distribution", fontsize=font_subtitle)  # set title of subfigure

# Add legend
sc_2015.set_array(values_2015)
legend = ax[0].legend(handles=[polygon_nat_2015, polygon_dra_2015], labels=["Undrained", 'Drained'], loc='lower right',
                      fontsize=font_legend)

# Add a, b, c, etc to the figures
plt.text(0.075, 0.9, 'A', bbox=dict(facecolor='white', edgecolor='black', boxstyle='square', pad=0.3, alpha=1.0),
         horizontalalignment='right', verticalalignment='bottom', transform=ax[0].transAxes, fontsize=font_text, weight='bold')
# # fig.tight_layout()
# plt.savefig(os.path.join(path_figs, "Map_2015.png"))
# plt.close()
...
m = Basemap(projection='mill', llcrnrlat=ll_lat, urcrnrlat=ur_lat,
            llcrnrlon=ll_lon, urcrnrlon=ur_lon, resolution='f', ax=ax[1])
m.drawcoastlines(linewidth=0.3)
...
cbar = plt.colorbar(im, ax=ax[1], extend='max', orientation='horizontal', shrink=0.4, aspect=30, pad=0.15)
ax[1].set_title('# of peatland fires [yr$^{-1}$ km$^{-2}$]', fontsize=font_subtitle)  # set title of subfigure
# cbar.ax.tick_params(labelsize=font_ticklabels)
# cbar.cmap.set_over('b')
# im.set_clim(0, 0.04) # Set the limits of the colorbar
# cbar.set_ticks([0, 0.01, 0.02, 0.03, 0.04])
cbar.remove()

# add statistics
mstats = 'Avg. = %.2f yr$^{-1}$ km$^{-2}$' % (np.nanmean(annual_ignitions))

if np.nanmean(lats) > 40:
    plt.text(1.0, 1.1, mstats, horizontalalignment='right', verticalalignment='bottom', transform=ax[1].transAxes,
             fontsize=font_text, bbox=dict(facecolor='white', edgecolor='black', boxstyle='square,pad=0.3'))
else:
    plt.text(0.5, 0.9, mstats, bbox=dict(facecolor='white', alpha=1.0), horizontalalignment='center',
             verticalalignment='bottom', transform=ax[1].transAxes, fontsize=font_text)

# Add a, b, c, etc to the figures
plt.text(0.075, 0.9, 'B', bbox=dict(facecolor='white', edgecolor='black', boxstyle='square', pad=0.3, alpha=1.0),
         horizontalalignment='right', verticalalignment='bottom', transform=ax[1].transAxes, fontsize=font_text, weight='bold')

plt.tight_layout()
# plt.savefig(os.path.join(path_figs, "Map_2015_fires.png"))
# plt.close()
plt.show()