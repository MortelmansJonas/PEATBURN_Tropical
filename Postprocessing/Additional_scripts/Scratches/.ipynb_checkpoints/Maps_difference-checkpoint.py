...
im = m.pcolormesh(lons, lats, np.nanmean(ds_exp1['MERRA2_FWI'][:] - ds_ref['MERRA2_FWI'][:], axis=0),
                  latlon=True, cmap=seismic_dis)
cbar = plt.colorbar(im, ax=ax, extend='both', orientation='horizontal', shrink=0.5, aspect=50, pad=0.15)
im.set_clim(-1, 3)

ax.set_title('FWI$_{peat}$ - FWI$_{ref}$', fontsize=font_subtitle)
...
for record, shape in zip(records_day1, shapes):
...
# plt.show()
plt.savefig(os.path.join(path_figs, "FWI_maps_difference_all.png"))
plt.close()