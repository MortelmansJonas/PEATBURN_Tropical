#!/usr/bin/env python
"""
This script makes maps of the FWI for different days with pastel colors and only marks fires for which there was a
change between FWIref and FWIpeat.
"""
...
for i, j, ds, index_day in [(0, 0, ds_ref, index_day_1), (0, 1, ds_exp1, index_day_1),
                            (1, 0, ds_ref, index_day_2), (1, 1, ds_exp1, index_day_2)]:

    # Create a specific colorbar based on the FWI threshold:
    # Create a colorbar for the FWI:
    colors = ['#8F92FF', '#A3FFA3', '#F8E396', '#FF8F8F']
    # Define the value ranges corresponding to each color (Ranges defined for Indonesia)
    threshold_50 = np.nanquantile(ds['MERRA2_FWI'][:], 0.5)
    threshold_70 = np.nanquantile(ds['MERRA2_FWI'][:], 0.7)
    threshold_90 = np.nanquantile(ds['MERRA2_FWI'][:], 0.9)
    print(threshold_50)
    print(threshold_70)
    print(threshold_90)
    boundaries = [0, threshold_50, threshold_70, threshold_90, 1000]
    cmap2 = ListedColormap(colors)
    norm = BoundaryNorm(boundaries, ncolors=cmap2.N, clip=True)
    ...
    im = m.pcolormesh(lons, lats, ds['MERRA2_FWI'][index_day, :, :], latlon=True, cmap=cmap2, norm=norm)
...
for i, label in enumerate(['(a)', '(b)', '(c)', '(d)']):
    plt.text(0.05, 0.9, label, bbox=dict(facecolor='white', alpha=1.0), horizontalalignment='right',
...
# plt.show()
plt.savefig(os.path.join(path_figs, "FWI_maps_thresholds_colors_507090.png"))
plt.close()