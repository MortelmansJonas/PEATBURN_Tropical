#!/usr/bin/env python
"""
This script makes maps of the FWI for different days.
"""
...
for i, j, ds, index_day in [(0, 0, ds_ref, index_day_1), (0, 1, ds_exp3b, index_day_1),
    ...
    im = m.pcolormesh(lons, lats, ds['MERRA2_FWI'][index_day, :, :], latlon=True, cmap=cmap2, norm=norm, alpha=0.5,
                      linewidth=0)
    ...
    for record, shape in zip(record_set, shapes):
        if record['M2H_EXP1'] == 1:
            ...
            lon, lat = shape.points[0]
            x, y = m(lon, lat)
            m.scatter(x, y, c=color, s=size, linewidth=0.1, marker=marker, edgecolor='white', zorder=3)
...
# plt.show()
plt.savefig(os.path.join(path_figs, "FWI_maps6.png"))
plt.close()