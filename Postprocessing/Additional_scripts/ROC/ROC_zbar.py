...
# DC:
ds_DC_nat = Dataset(os.path.join(path_out, 'CDF_matched_DC_MERRA2_FWI_Nat_pixel.nc'), 'r')
DC_M2_nat = ds_DC_nat['MERRA2_FWI'][0:6209, :, :].data
...
# DC:
ds_DC_dra = Dataset(os.path.join(path_out, 'CDF_matched_DC_MERRA2_FWI_Dra_pixel.nc'), 'r')
DC_M2_dra = ds_DC_dra['MERRA2_FWI'][0:6209, :, :].data
...
## zbar:
fpr_zbar_nat, tpr_zbar_nat, auc_zbar_nat, tpr_lower_zbar_nat, tpr_upper_zbar_nat, auc_all_zbar_nat = bootstrap_roc(fires_raster_nat, -zbar_nat, acl=acl_zbar_nat)

ax['A'].plot(fpr_zbar_nat, tpr_zbar_nat, color='tab:green', label='zbar (AUC = %0.2f)' % auc_zbar_nat, linewidth=1)
ax['A'].fill_between(fpr_zbar_nat, tpr_lower_zbar_nat, tpr_upper_zbar_nat, color='tab:green', alpha=0.2)
...
## zbar:
fpr_zbar_dra, tpr_zbar_dra, auc_zbar_dra, tpr_lower_zbar_dra, tpr_upper_zbar_dra, auc_all_zbar_dra = bootstrap_roc(fires_raster_dra, -zbar_dra, acl=acl_zbar_dra)

ax['B'].plot(fpr_zbar_dra, tpr_zbar_dra, color='tab:green', label='zbar (AUC = %0.2f)' % auc_zbar_dra, linewidth=1)
ax['B'].fill_between(fpr_zbar_dra, tpr_lower_zbar_dra, tpr_upper_zbar_dra, color='tab:green', alpha=0.2)
...
# plt.savefig(os.path.join(path_figs, 'ROC_EXP3b_bootstrapped'))
plt.show()
...
DC_nat_stat, DC_nat_p = shapiro(auc_all_DC_nat)
DC_dra_stat, DC_dra_p = shapiro(auc_all_DC_dra)
...
print("DC, natural p-value: {}".format(DC_nat_p))
print("DC, drained p-value: {}".format(DC_dra_p))

z_stat1, p_value1 = ztest(auc_all_M2_nat, auc_all_EXP3b_nat)
z_stat2, p_value2 = ztest(auc_all_M2_nat, auc_all_DC_nat)
z_stat3, p_value3 = ztest(auc_all_DC_nat, auc_all_EXP3b_nat)

z_stat4, p_value4 = ztest(auc_all_M2_dra, auc_all_EXP3b_dra)
z_stat5, p_value5 = ztest(auc_all_M2_dra, auc_all_DC_dra)
z_stat6, p_value6 = ztest(auc_all_DC_dra, auc_all_EXP3b_dra)

print("=== NATURAL ===")
print(f"FWIref versus FWIpeat:\nZ-statistic: {z_stat1:.2f}, P-value: {p_value1:.4f}")
print(f"FWIref versus DCref:\nZ-statistic: {z_stat2:.2f}, P-value: {p_value2:.4f}")
print(f"DCref versus FWIpeat:\nZ-statistic: {z_stat3:.2f}, P-value: {p_value3:.4f}")

print("=== DRAINED ===")
print(f"FWIref versus FWIpeat:\nZ-statistic: {z_stat4:.2f}, P-value: {p_value4:.4f}")
print(f"FWIref versus DCref:\nZ-statistic: {z_stat5:.2f}, P-value: {p_value5:.4f}")
print(f"DCref versus FWIpeat:\nZ-statistic: {z_stat6:.2f}, P-value: {p_value6:.4f}")
