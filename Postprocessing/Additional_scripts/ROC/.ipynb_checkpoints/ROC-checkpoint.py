#!/usr/bin/env python
"""
This script is used to calculate the ROC curve for the different FWI calculations. The stratification in time (early
vs. late fires) will not be necessary over the tropics. Instead,
...
# ---------------------------------------------------------------------------------------------
for CDF_type in CDF_types:  # Loop over domain-wise and pixel-wise CDF matching methods
    # ---------------------------------------------------------------------------------------------
    # LOAD FWI DATASETS
    # ---------------------------------------------------------------------------------------------
    ## Natural
    ...
    # EXP1:
    ds_EXP1_nat = Dataset(os.path.join(path_out, 'FWI_zbar_DC_TN_' + CDF_type + '.nc'), 'r')
    FWI_EXP1_nat = ds_EXP1_nat['MERRA2_FWI'][0:6209, :, :].data

    # EXP2:
    ds_EXP2_nat = Dataset(os.path.join(path_out, 'FWI_sfmc_DMC_TN_' + CDF_type + '.nc'), 'r')
    FWI_EXP2_nat = ds_EXP2_nat['MERRA2_FWI'][0:6209, :, :].data

    # EXP3:
    ds_EXP3_nat = Dataset(os.path.join(path_out, 'FWI_sfmc_FFMC_TN_' + CDF_type + '.nc'), 'r')
    FWI_EXP3_nat = ds_EXP3_nat['MERRA2_FWI'][0:6209, :, :].data

    # EXP4:
    ds_EXP4_nat = Dataset(os.path.join(path_out, 'FWI_zbar_FWI_TN_' + CDF_type + '.nc'), 'r')
    FWI_EXP4_nat = ds_EXP4_nat['zbar'][0:6209, :, :].data

    # EXP2b:
    ds_EXP2b_nat = Dataset(os.path.join(path_out, 'FWI_zbar_DMC_TN_' + CDF_type + '.nc'), 'r')
    FWI_EXP2b_nat = ds_EXP2b_nat['MERRA2_FWI'][0:6209, :, :].data

    # EXP3b:
    ds_EXP3b_nat = Dataset(os.path.join(path_out, 'FWI_zbar_FFMC_TN_' + CDF_type + '.nc'), 'r')
    ...
    # EXP1:
    ds_EXP1_dra = Dataset(os.path.join(path_out, 'FWI_zbar_DC_TD_' + CDF_type + '.nc'), 'r')
    FWI_EXP1_dra = ds_EXP1_dra['MERRA2_FWI'][0:6209, :, :].data

    # EXP2:
    ds_EXP2_dra = Dataset(os.path.join(path_out, 'FWI_sfmc_DMC_TD_' + CDF_type + '.nc'), 'r')
    FWI_EXP2_dra = ds_EXP2_dra['MERRA2_FWI'][0:6209, :, :].data

    # EXP3:
    ds_EXP3_dra = Dataset(os.path.join(path_out, 'FWI_sfmc_FFMC_TD_' + CDF_type + '.nc'), 'r')
    FWI_EXP3_dra = ds_EXP3_dra['MERRA2_FWI'][0:6209, :, :].data

    # EXP4:
    ds_EXP4_dra = Dataset(os.path.join(path_out, 'FWI_zbar_FWI_TD_' + CDF_type + '.nc'), 'r')
    FWI_EXP4_dra = ds_EXP4_dra['zbar'][0:6209, :, :].data

    # EXP2b:
    ds_EXP2b_dra = Dataset(os.path.join(path_out, 'FWI_zbar_DMC_TD_' + CDF_type + '.nc'), 'r')
    FWI_EXP2b_dra = ds_EXP2b_dra['MERRA2_FWI'][0:6209, :, :].data

    # EXP3b:
    ds_EXP3b_dra = Dataset(os.path.join(path_out, 'FWI_zbar_FFMC_TD_' + CDF_type + '.nc'), 'r')
    ...
    threshold_EXP2b_nat = np.nanquantile(FWI_EXP2b_nat, 0.9)
    ...
    threshold_EXP2b_dra = np.nanquantile(FWI_EXP2b_dra, 0.9)
    ...
    A_EXP2b_nat, B_EXP2b_nat, C_EXP2b_nat, D_EXP2b_nat = Contingency_table(FWI_EXP2b_nat, fires_raster_nat,
                                                                           threshold_EXP2b_nat, 1)

    A_EXP3b_nat, B_EXP3b_nat, C_EXP3b_nat, D_EXP3b_nat = Contingency_table(FWI_EXP3b_nat, fires_raster_nat,
                                                                           threshold_EXP3b_nat, 1)
    ...
    TPR_EXP2b_nat, _, _, _, _, FPR_EXP2b_nat = CT_metrics(A_EXP2b_nat, B_EXP2b_nat, C_EXP2b_nat, D_EXP2b_nat)
    TPR_EXP3b_nat, _, _, _, _, FPR_EXP3b_nat = CT_metrics(A_EXP3b_nat, B_EXP3b_nat, C_EXP3b_nat, D_EXP3b_nat)
    ...
    A_EXP2b_dra, B_EXP2b_dra, C_EXP2b_dra, D_EXP2b_dra = Contingency_table(FWI_EXP2b_dra, fires_raster_dra,
                                                                           threshold_EXP2b_dra, 1)
    A_EXP3b_dra, B_EXP3b_dra, C_EXP3b_dra, D_EXP3b_dra = Contingency_table(FWI_EXP3b_dra, fires_raster_dra,
                                                                           threshold_EXP3b_dra, 1)
    ...
    TPR_EXP2b_dra, _, _, _, _, FPR_EXP2b_dra = CT_metrics(A_EXP2b_dra, B_EXP2b_dra, C_EXP2b_dra, D_EXP2b_dra)
    TPR_EXP3b_dra, _, _, _, _, FPR_EXP3b_dra = CT_metrics(A_EXP3b_dra, B_EXP3b_dra, C_EXP3b_dra, D_EXP3b_dra)
    ...
    axes[0].plot(fpr_M2_nat, tpr_M2_nat, color=c_ref, ls=ls_ref, label='FWI$_{ref}$ (AUC = %0.2f)' % auc_M2_nat)
    ...
    axes[0].plot(fpr_EXP1_nat, tpr_EXP1_nat, color=c_EXP1, ls=ls_EXP1, label='EXP1 (AUC = %0.2f)' % auc_EXP1_nat)
    ...
    axes[0].plot(fpr_EXP2_nat, tpr_EXP2_nat, color=c_EXP2, ls=ls_EXP2, label='EXP2 (AUC = %0.2f)' % auc_EXP2_nat)
    ...
    axes[0].plot(fpr_EXP3_nat, tpr_EXP3_nat, color=c_EXP3, ls=ls_EXP3, label='EXP3 (AUC = %0.2f)' % auc_EXP3_nat)
    ...
    axes[0].plot(fpr_EXP4_nat, tpr_EXP4_nat, color=c_EXP4, ls=ls_EXP4, label='EXP4 (AUC = %0.2f)' % auc_EXP4_nat)
    ...
    axes[0].plot(fpr_EXP2b_nat, tpr_EXP2b_nat, color=c_EXP2b, ls=ls_EXP2b, label='EXP2b (AUC = %0.2f)' %
                                                                                 auc_EXP2b_nat)
    ...
    axes[0].plot(fpr_EXP3b_nat, tpr_EXP3b_nat, color=c_EXP3b, ls=ls_EXP3b, label='EXP3b (AUC = %0.2f)' %
                                                                                 auc_EXP3b_nat)
    ...
    #
    # ## EXP3:
    # axes[0].scatter(FPR_EXP3_nat, TPR_EXP3_nat, color='tab:brown', marker='*', s=70,
    #                 edgecolor='k', linewidths=0.5, zorder=2.5)
    #
    # ## EXP4:
    ...
    #
    # ## EXP2b:
    # axes[0].scatter(FPR_EXP2b_nat, TPR_EXP2b_nat, color=palette[2], marker='d', s=70,
    #                 edgecolor='k', linewidths=0.5, zorder=2.5)
    #
    # ## EXP3b:
    # axes[0].scatter(FPR_EXP3b_nat, TPR_EXP3b_nat, color='tab:brown', marker='d', s=70,
    #                 edgecolor='k', linewidths=0.5, zorder=2.5)
    ...
    axes[1].plot(fpr_M2_dra, tpr_M2_dra, color=c_ref, ls=ls_ref, label='FWI$_{ref}$ (AUC = %0.2f)' % auc_M2_dra)
    ...
    axes[1].plot(fpr_EXP1_dra, tpr_EXP1_dra, color=c_EXP1, ls=ls_EXP1, label='EXP1 (AUC = %0.2f)' % auc_EXP1_dra)
    ...
    axes[1].plot(fpr_EXP2_dra, tpr_EXP2_dra, color=c_EXP2, ls=ls_EXP2, label='EXP2 (AUC = %0.2f)' % auc_EXP2_dra)
    ...
    axes[1].plot(fpr_EXP3_dra, tpr_EXP3_dra, color=c_EXP3, ls=ls_EXP3, label='EXP3 (AUC = %0.2f)' % auc_EXP3_dra)
    ...
    axes[1].plot(fpr_EXP4_dra, tpr_EXP4_dra, color=c_EXP4, ls=ls_EXP4, label='EXP4 (AUC = %0.2f)' % auc_EXP4_dra)
    ...
    axes[1].plot(fpr_EXP2b_dra, tpr_EXP2b_dra, color=c_EXP2b, ls=ls_EXP2b, label='EXP2b (AUC = %0.2f)' %
                                                                                 auc_EXP2b_dra)
    ...
    axes[1].plot(fpr_EXP3b_dra, tpr_EXP3b_dra, color=c_EXP3b, ls=ls_EXP3b, label='EXP3b (AUC = %0.2f)' %
                                                                                 auc_EXP3b_dra)
    ...
    #
    # ## EXP3:
    # axes[1].scatter(FPR_EXP3_dra, TPR_EXP3_dra, color='tab:brown', marker='*', s=70,
    #                 edgecolor='k', linewidths=0.5, zorder=2.5)
    #
    # ## EXP4:
    ...
    #
    # ## EXP2b:
    # axes[1].scatter(FPR_EXP2b_dra, TPR_EXP2b_dra, color=palette[2], marker='d', s=70,
    #                 edgecolor='k', linewidths=0.5, zorder=2.5)
    #
    # ## EXP3b:
    # axes[1].scatter(FPR_EXP3b_dra, TPR_EXP3b_dra, color='tab:brown', marker='d', s=70,
    #                 edgecolor='k', linewidths=0.5, zorder=2.5)
    ...
    fig.tight_layout()
    plt.savefig(os.path.join(path_figs, 'ROC_all_' + CDF_type))
